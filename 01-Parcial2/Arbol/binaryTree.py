...
class BinaryTree:
    ...
    def search_by_coincidence_two(self, value):
        def __search_by_coincidence_two(root, value):
            if root is not None:
                if root.value.startswith(value):
                    return root.value
                __search_by_coincidence_two(root.left, value)
                __search_by_coincidence_two(root.right, value)

        __search_by_coincidence_two(self.root, value)
    ...
    def inorden_tipo_agua(self):
        def __inorden_tipo_agua(root):
            if root is not None:
                __inorden_tipo_agua(root.left)
                if root.other_values and root.other_values.get('tipos') == 'Agua':
                    print(root.other_values.get('name'))
                __inorden_tipo_agua(root.right)

        __inorden_tipo_agua(self.root)
        
    def contar_tipos(self, value):
        def __contar_tipos(root, value):
            count = 0
            if root is not None:
                if root.value == value:
                    count = 1
                count += __contar_tipos(root.left, value)
                count += __contar_tipos(root.right, value)
            return count

        return __contar_tipos(self.root, value)